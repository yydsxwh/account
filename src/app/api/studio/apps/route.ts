...
import {
  ...
  validateHomepageUrl,
} from "@/helpers/studio-app-update";
...
export async function POST(req: Request) {
  try {
    ...
    const homepage = validateHomepageUrl(body.homepageUrl);
    if (!homepage.ok) {
      return NextResponse.json({ error: homepage.error }, { status: 400 });
    }
...
export async function PATCH(req: Request) {
  try {
    ...
    assertNoIdentityMutation(raw);
    const body = studioAppPatchFieldsSchema.parse(raw);
    ...
    const data = buildStudioAppPrismaData(body);
    const allowedScopes = checkScopes(body.allowedScopes);
    if (allowedScopes) {
      (data as { allowedScopes?: string }).allowedScopes = allowedScopes;
    }
    if (body.grantTypes) {
      (data as { grantTypes?: string }).grantTypes = body.grantTypes.join(" ");
    }
    if (body.rotateSecret) {
      plaintextSecret = generateClientSecret();
      (data as { clientSecret?: string }).clientSecret =
        await hashClientSecret(plaintextSecret);
    }
    ...
    return NextResponse.json({
      ...
      message: plaintextSecret ? "新密钥已生成，请立刻保存" : "保存成功",
    });
  } catch (error) {
    const mapped = studioErrorResponse(error);
    return NextResponse.json({ error: mapped.error }, { status: mapped.status });
