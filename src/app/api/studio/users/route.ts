...
export async function GET(req: Request) {
  try {
    ...
    const users = await prisma.user.findMany({
      ...
      orderBy: pendingOnly ? { createdAt: "asc" } : { createdAt: "desc" },
      take: 200,
    });
  ...
  try {
    ...
    if (raw?.unbindWechat === true) {
      ...
      if (
        ...
        !target.wechatMobileOpenId?.trim() &&
    ...
    if ("applicationAction" in raw) {
      ...
      if (body.applicationAction === "approve") {
        const updated = await prisma.user.update({
          where: { id: body.userId },
          data: fieldsForApprove(target.requestedRole, admin.id, body.note, {
            role: target.role,
            roles: target.roles,
          }),
          select: userSelect,
    ...
    if (
      ...
      if (adminCount <= 1) {
        return NextResponse.json({ error: "至少保留一位站长" }, { status: 400 });
      }
    }
